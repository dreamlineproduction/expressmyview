...
$(function() {
  ...
  $("#statusScreen").show();
  $("#statusScreen span").html(
    "Host has not joined yet or left the broadcast."
  );
  ...
  audienceStore.subscribe(() => {
    ...
    if (audienceState.connectionState === "CONNECTED") {
      ...
      $("#statusScreen").show();
      $("#statusScreen span").html(
        "You are connected to the live stream! Host is yet to join."
      );
      spinnerDiv.show();
    }
    if (audienceState.connectionState !== "CONNECTED") {
  ...
  hostsStore.subscribe(() => {
    ...
    if (!viewersState.hostConnected) {
      ...
      $("#statusScreen").show();
      $("#statusScreen span").html(
        "Host got disconnected or left the broadcast"
      );
      spinnerDiv.show();
    }
  ...
  async function JoinChat() {
    ...
    RTM.rtmclient
      .login({ token: tokenrtm, uid: userrtm })
      .then(() => {
        RTM.loggedIn = true;
  ...
  async function joinLiveStream() {
    ...
    bclient.client.on("user-published", async (user, mediaType) => {
      try {
        ...
        if (mediaType === "video") {
          ...
          const playerDiv = $("<div>", { id: idvname });
          document.createElement("div");
          const videodiv = $("<video />", { id: "fluid_" + idvname });
          playerDiv.append(videodiv);
          $("#external-broadcasts-container").append(playerDiv);

          const track = remoteVideoTrack.getMediaStreamTrack();
          const stream = new MediaStream();
          stream.addTrack(track);
          const playerdiv = document.getElementById("fluid_" + idvname);
          playerdiv.srcObject = stream;
          const ff = fluidPlayer("fluid_" + idvname, {
            layoutControls: {
              posterImage: thumbnailurl,
              playButtonShowing: false,
              autoPlay: true,
          ...
          hostTracks[user.uid].push({ divname: idvname, mediaType, ff });
    ...
    bclient.client.on("user-unpublished", (user, mediaType) => {
      ...
      const divname = hostTracks[user.uid].filter(lld => {
        return lld.mediaType === mediaType;
      });
      const newlist = hostTracks[user.uid].filter(lld => {
        return lld.mediaType !== mediaType;
      });
    ...
    const uid = await bclient.client.join(
      options.appId,
      options.channel,
      options.token,
      null
  ...
  async function leaveCall() {
    for (const hostid in hostTracks) {
      if (hostTracks[hostid].length > 0) {
        hostTracks[hostid].forEach((val, idx) => {
          const { divname, mediaType, ff } = val;
          if (mediaType === "video") {
            hostsStore.dispatch({ type: "DECREASE_VTRACK_COUNT" });
            ff.destroy();
          }
