                        ...
                        <div class="single-video">
                            @if($podcast->converted == 0)
                                <div class="proccessing-section text-center single-video-title box mb-2">
                                    <svg aria-hidden="true" focusable="false" data-prefix="fas" width="40"
                            ...
                            @else
                                ...
                                @else
                                    <div id="single-audio"
                                         data-source="{{ url('/storage/podcast/audio/' . $podcast->filename) }}"></div>
                        ...
                        <div class="single-video-author box mb-2">
                            <div class="float-right">
                                ...
                                @if(Auth::check())
                                    ...
                                    @else
                                        <button class="btn btn btn-outline-danger" type="button"><i
                                                    class="far fa-thumbs-up"></i> {{ formatSubscribersCount($podcast->likes) }}
                                        </button>
                                    @endif
                                @else
                                    <a href="{{ route('login') }}" class="btn btn-danger" role="button">
                            ...
                            <img class="img-fluid"
                                 src="{{ $podcast->channel->logo ? Storage::disk('s3')->url('public/users/logo/' . $podcast->channel->logo) : asset('img/user.jpg') }}"
                                 alt="">
                            <p>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="gedf-card">
                                        ...
                                        @if(Auth::check())
                                            <div class="card-body1">
                                                <form action="{{ route('comment.store', $podcast->id) }}"
                                                      id="comment_form">
                                                    @csrf
                                    ...
                                    <div id="comments">
                                        @foreach($comments as $comment)
                                            <div class="card gedf-card mt-4 comment-single"
                                                        ...
                                                        <div class="d-flex justify-content-between align-items-center">
                                                            <div class="mr-2">
                                                                <img class="rounded-circle" width="45"
                                                                     src="{{ $comment->user->profile->avatar ? Storage::disk('s3')->url('public/users/avatar/' . $comment->user->profile->avatar) : asset('img/user.png') }}"
                                                                     alt="Avatar">
                                                            </div>
                                                            <div class="ml-2">
                                                ...
                                                <div class="card-footer">
                                                    @if(Auth::check())
                                                        <a href="#"
                                                           class="card-link {{ Auth::user()->id != $comment->user_id && !isCommentLiked($comment->id) ? 'like-comment' : '' }}">
                                                            <i class="fa fa-gittip"></i> <span class="like-count-txt">{{ formatViewsCount($comment->likes, 1) }}</span> <span class="like-txt">{{ isCommentLiked($comment->id) ? 'Liked' : 'Like' }}</span>
                                                        </a>
                                                    @else
                                                    ...
                                                    @if(Auth::check() && Auth::user()->id == $podcast->user_id)
                                                        <a href="#"
                                                           class="card-link delete-comment float-right">Delete</a>
                                                    @endif
                                                </div>
                                            </div>
                                        @endforeach
                                ...
                                @foreach($relatedPodcasts as $podcast)
                                    <div class="video-card video-card-list">
                                        <div class="video-card-image">
                                            <a class="play-icon" href="{{ route('podcast.show', $podcast->id) }}"><i class="fas fa-play-circle"></i></a>
                                            <a href="{{ route('podcast.show', $podcast->id) }}"><img class="img-fluid" src="{{ Storage::disk('s3')->url('public/podcast/thumbnail/' . $podcast->thumbnail) }}" alt=""></a>
                                        ...
                                        <div class="video-card-body">
                                            ...
                                            <div class="video-page text-success">
                                                <a href="{{ route('channel.show', $podcast->channel_id) }}">{{ $podcast->channel->name }}</a>
                                                @if($podcast->channel->verified)
                                            ...
                                            <div class="video-view">
                                                {{ formatViewsCount($podcast->views, 1) }} views &nbsp;<i class="fas fa-calendar-alt"></i> {{ $podcast->created_at->diffForHumans() }}
                                            </div>
                                        </div>
                                    </div>
                                @endforeach
