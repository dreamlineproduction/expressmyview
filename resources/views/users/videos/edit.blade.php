...
<div class="container-fluid upload-details">
    ...
    <div class="row">
        ...
        <div class="col-lg-2">
            <div class="imgplace"
                style="background-image: url('{{ Storage::disk('s3')->url('public/podcast/thumbnail/' . $video->thumbnail) }}">
            </div>
        </div>
    </div>
    <hr>
    <div class="row">
        <div class="col-lg-12">
            <form action="{{ route('podcast.update', $video->id) }}" method="post" id="podcast_details">
                @csrf
                @method('put')
                            ...
                            <div class="form-group">
                                ...
                                <select name="channel" id="channel" class="form-control" required>
                                    ...
                                    <option value="{{ $channel->id }}"
                                        {{ $channel->id == $video->channel_id ? ' selected' : '' }}>{{ $channel->name }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div class="col-lg-12">
                            <div class="form-group">
                                ...
                                <input type="text" placeholder="Contrary to popular belief, Lorem Ipsum (2019) is not."
                                    name="title" id="title" value="{{ $video->title }}" class="form-control" required>
                            </div>
                        </div>
                        <div class="col-lg-12">
                            <div class="form-group">
                        ...
                        <div class="col-lg-3">
                            <div class="form-group">
                                ...
                                <select name="privacy" id="privacy" class="custom-select" required>
                                    <option value="1" {{ $video->privacy == 1 ? ' selected' : '' }}>Public</option>
                                    <option value="0" {{ $video->privacy == 0 ? ' selected' : '' }}>Private</option>
                            ...
                            <div class="form-group">
                                ...
                                <select name="license" id="license" class="custom-select" required>
                                    ...
                                    <option value="{{ $license->id }}"
                                        {{ $video->license_id == $license->id ? ' selected' : '' }}>{{ $license->name }}
                                    </option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="languages[]" id="languages" class="custom-select" multiple required>
                                    ...
                                    <option value="{{ $language->id }}"
                                        {{ $language->code === 'en' ? ' selected' : '' }}>{{ $language->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                                ...
                                <div class="mt-2">
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="comments_allowed"
                                            id="commentsAllowed" value="1"
                                            {{ $video->comments_allowed == 1 ? 'checked ' : '' }}required>
                                        <label class="custom-control-label" for="commentsAllowed">Yes</label>
                                    </div>
                                    <div class="custom-control custom-radio custom-control-inline">
                                        <input type="radio" class="custom-control-input" name="comments_allowed"
                                            id="commentsNotAllowed" value="0"
                                            {{ $video->comments_allowed == 0 ? 'checked ' : '' }}>
